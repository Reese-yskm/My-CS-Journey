...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    all_low = [lower(s) for s in subject]

    def res(para):
        final = split(remove_punctuation(lower(para)))
        for s in all_low:
            if s in final:
                return True
        return False
...
def accuracy(typed, source):
    ...
    if not source_words:
        return 0.0
    right = 0
    base = min(len(typed_words), len(source_words))
    for i in range(base):
        if typed_words[i] == source_words[i]:
            right = right + 1
    return (float)((right / len(typed_words)) * 100)
    # END PROBLEM 3
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]:  # Feel free to remove or add additional cases
        ...
        # BEGIN
        res = 1 + min(add, remove, substitute)
        return res
        # END
